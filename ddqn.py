...
if __name__ == "__main__":
    ...
    for i in range(5):
        agent = DDQNAgent(env, memory_size, batch_size, target_update, epsilon_decay)
        scores, losses, epsilons= agent.train(num_frames)
        with open("./save/ddqn/scores_" + str(i) + ".txt", encoding="utf-8", mode="a") as f:
            f.writelines(str(scores))
        with open("./save/ddqn/losses_" + str(i) + ".txt", encoding="utf-8", mode="a") as f:
            f.writelines(str(losses))
        with open("./save/ddqn/epsilons_" + str(i) + ".txt", encoding="utf-8", mode="a") as f:
            f.writelines(str(epsilons))
        agent.save("./save/ddqn/ddqn_" + str(i) + ".pkl")
    # agent = DDQNAgent(env, memory_size, batch_size, target_update, epsilon_decay)
    # agent.load("./save/ddqn/ddqn_2.pkl")
    # video_folder = "videos/ddqn"
